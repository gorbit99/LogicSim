...
Node node_create(char *compName, Point pos, TTF_Font *font, SDL_Renderer *renderer) {
	...
	if (strcmp_nocase(compName, "LED"))
		node.component.type = CT_LED;
	else if (strcmp_nocase(compName, "SWITCH"))
...
void node_render(Node *node, Point camPos) {
	for (int i = 0; i < node->component.funData.outC; i++) {
		for (size_t w = 0; w < node->wires[i].conCount; w++) {
			if (node->outValues[i])
				component_render(&node->wires[i].connections[w].wireComp, node->renderer, camPos, 0xd1db14ff);
			else
				component_render(&node->wires[i].connections[w].wireComp, node->renderer, camPos, 0x4c0082ff);
		}
	}
	if (node->component.type == CT_MODULE)
		...
		if (node->inValues[0])
			component_render(&node->component, node->renderer, camPos, 0xd1db14ff);
		else
			component_render(&node->component, node->renderer, camPos, 0x4c0082ff);
